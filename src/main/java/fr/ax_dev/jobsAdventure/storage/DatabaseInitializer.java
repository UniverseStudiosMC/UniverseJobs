...
public class DatabaseInitializer {
     ...
     */
    public DatabaseInitializer(JobsAdventure plugin, ConnectionPool connectionPool) {
        this.plugin = plugin;
    ...
    private void executeScript(String script) throws Exception {
        ...
        try {
            // Split script by semicolon, but be careful with stored procedures
            ...
            try (Statement stmt = connection.createStatement()) {
                for (String sql : statements) {
                    ...
                    try {
                        stmt.execute(sql);
                        plugin.getLogger().info("Executed SQL: " + (sql.length() > 50 ? sql.substring(0, 50) + "..." : sql));
                    } catch (Exception e) {
                        // Log warning but continue with other statements
                        plugin.getLogger().log(Level.WARNING, "Failed to execute SQL statement: " + sql, e);
                    }
                }
            }
            ...
            try (Statement stmt = connection.createStatement()) {
                for (String indexSql : indexStatements) {
                    try {
                        ...
                        plugin.getLogger().info("Created index: " + indexSql);
                    } catch (Exception e) {
                        // Index might already exist, log as warning
                        plugin.getLogger().log(Level.WARNING, "Failed to create index: " + indexSql, e);
                    }
                }
