...
public class AdminCommandHandler extends JobCommandHandler {
    ...
    public AdminCommandHandler(JobsAdventure plugin) {
        super(plugin);
    }
    ...
    public List<String> getTabCompletions(CommandSender sender, String[] args) {
        ...
        if (args.length == 2) {
            ...
            switch (subCommand) {
                case "exp" -> {
                    if (hasPermission(sender, "jobsadventure.admin.exp")) {
                ...
                case "debug" -> {
                    if (hasPermission(sender, "jobsadventure.admin.debug")) {
    ...
    private void handleExpCommand(CommandSender sender, String[] args) {
        ...
        if (targetPlayer == null) {
            MessageUtils.sendMessage(sender, "&cPlayer " + playerName + " not found or not online.");
            return;
        }
        ...
        if (!jobManager.hasJob(targetPlayer, jobId)) {
            MessageUtils.sendMessage(sender, "&cPlayer " + playerName + " doesn't have the job " + job.getName());
            return;
        }
        ...
        switch (expAction.toLowerCase()) {
            case "give" -> {
                ...
                MessageUtils.sendMessage(sender, "&aGave " + amount + " XP to " + playerName + " for job " + job.getName());
            ...
            case "take" -> {
                ...
                MessageUtils.sendMessage(sender, "&aTook " + amount + " XP from " + playerName + " for job " + job.getName());
            ...
            case "set" -> {
                ...
                MessageUtils.sendMessage(sender, "&aSet " + playerName + "'s XP to " + amount + " for job " + job.getName());
    ...
    private void handleMigrateCommand(CommandSender sender, String[] args) {
        if (!hasPermission(sender, "jobsadventure.admin.migrate")) {
            sender.sendMessage(languageManager.getMessage("commands.no-permission"));
            return;
        }
    ...
    private void handleReloadCommand(CommandSender sender) {
        if (!hasPermission(sender, "jobsadventure.admin.reload")) {
            sender.sendMessage(languageManager.getMessage("commands.no-permission"));
            return;
        }
    ...
    private List<String> getAdminTabCompletions(CommandSender sender, String[] args) {
        ...
        if (args.length == 3) {
            ...
            if ("exp".equals(subCommand) && hasPermission(sender, "jobsadventure.admin.exp")) {
