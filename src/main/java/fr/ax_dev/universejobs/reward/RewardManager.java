    ...
    public boolean claimReward(Player player, Reward reward) {
        ...
        if (reward.hasEconomyReward()) {
            if (plugin.getServer().getPluginManager().getPlugin("Vault") == null) {
                plugin.getLogger().warning("Economy reward of " + reward.getEconomyReward() + " could not be given to " + player.getName() + " - Vault is not installed");
            } else {
                try {
                    Class<?> economyClass = Class.forName("net.milkbowl.vault.economy.Economy");
                    org.bukkit.plugin.RegisteredServiceProvider<?> rsp = plugin.getServer().getServicesManager().getRegistration(economyClass);

                    if (rsp != null) {
                        Object economy = rsp.getProvider();
                        economy.getClass().getMethod("depositPlayer", org.bukkit.OfflinePlayer.class, double.class)
                            .invoke(economy, player, reward.getEconomyReward());
                        if (plugin.getConfigManager().isDebugEnabled()) {
                            plugin.getLogger().info("Economy reward of " + reward.getEconomyReward() + " given to player " + player.getName());
                        }
                    } else {
                        plugin.getLogger().warning("Economy reward of " + reward.getEconomyReward() + " could not be given to " + player.getName() + " - No economy provider found");
                    }
                } catch (ClassNotFoundException e) {
                    plugin.getLogger().warning("Economy reward could not be given - Vault classes not found");
                } catch (Exception e) {
